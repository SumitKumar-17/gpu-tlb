 ...
 *
 ******************************************************************************/
Page::Page(MemDump &dump, std::uint64_t addr, TransType type, std::uint8_t flag)
    : Trans(dump, addr, type, flag) {
}
 ...
 *
 ******************************************************************************/
void Page::printTrans(std::uint64_t virtAddr) {
    ...
    std::cout << "\t|V:" << validFlag << "|AP:"
              << (aperture == APERTURE_VIDEO_LOCAL              ? "VL"
                  : aperture == APERTURE_VIDEO_PEER             ? "VP"
                  : aperture == APERTURE_SYSTEM_MEMORY_COHERENT ? "SC"
                                                                : "SN")
              << "|VOL:" << volatileFlag << "|E:" << encryptedFlag << "|P:" << priviledgedFlag
              << "|RO:" << readOnlyFlag << "|AD:" << atomicDisableFlag << "|";
    std::cout << std::endl;
}
