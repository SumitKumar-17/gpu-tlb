...
#define PAGE0_NUM 20000
#define PAGE1_NUM 10000
#define PAGE2_NUM 16
...
#define SMID0 0
#define SMID1 2 // IMPORTANT: SM0 and SM3 are in different GPCs on RTX3080

__global__ void loop(volatile uint64_t *page0, volatile uint64_t *page1, volatile uint64_t *page2,
                     uint64_t x) {
    uint64_t y = x;
...
l2_l3_eviction:
    ...
    clk0 = clock64();
    ptr = (uint64_t *)page0[0];
    clk1 = clock64();
    asm volatile("membar.cta;");

    // printf("ptr assignment time: %lu, page0[0]: 0x%lx, page0: 0x%lx\n", clk1 - clk0, (uint64_t)page0[0],
    // (uint64_t)page0);
    printf("time: %lu\n", clk1 - clk0);
    for (; ptr != page0; ptr = (uint64_t *)ptr[0]) {
        ++ptr[2];
    }
