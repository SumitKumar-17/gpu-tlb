...
std::vector<int> l2_idx_vec = {
    131628, 131757, 131886, 132015, 132128, 132257, 132386, 132515,
};

/*******************************************************************************
 * derive an L3-uTLB set
 ******************************************************************************/
__global__ void loop(volatile uint64_t *chain, volatile uint64_t *evict, volatile uint64_t *fill,
                     uint64_t x) {
    uint64_t y = x;
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (std::find(l2_idx_vec.begin(), l2_idx_vec.end(), k) != l2_idx_vec.end())
            continue;
        chain.push_back((uint64_t *)(BASE_ADDR + k * STRIDE_SIZE));
    }
