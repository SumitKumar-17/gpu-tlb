 ...
 *
 ******************************************************************************/
PageMap::PageMap(MemDump &dump, std::uint64_t addr, TransType type) : Trans(dump, addr, type) {
}
...
bool PageMap::constructTrans() {
    ...
    for (int i = 0; i < entNum; ++i) {
        ...
        TransType nTransType = mTransType == PD3 ? PD2 : mTransType == PD2 ? PD1 : PD0;
 ...
 *
 ******************************************************************************/
void PageMap::printTrans(std::uint64_t virtAddr) {
    if (mTransType == PD3)
        std::cout << "PD3@0x";
    ...
    std::cout << std::hex << std::setw(10) << std::setfill('0') << mPhyAddr << std::endl;
