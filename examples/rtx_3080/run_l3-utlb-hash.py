...
dumper_path = '/home/rtx3060/Desktop/gpu-tlb/dumper/dumper'
extractor_path = '/home/rtx3060/Desktop/gpu-tlb/extractor/extractor'
modifier_path = '/home/rtx3060/Desktop/gpu-tlb/modifier/modifier'
...
a_out = subprocess.Popen(['./l3-utlb-hash', str(target)])
...
for bit in range(7, 47):
  a_out = subprocess.Popen(['./l3-utlb-hash', str(target), str(bit)])
  b_out = subprocess.Popen(['nvidia-smi', '-f', '/dev/null'])
  b_out.wait() # in case the driver warms up very slowly
  time.sleep(2)
...
for i in range(len(hash_bit)):
  for j in range(i + 1, len(hash_bit)):
    ...
    a_out = subprocess.Popen(['./l3-utlb-hash', str(target), str(bit0), str(bit1)])
    b_out = subprocess.Popen(['nvidia-smi', '-f', '/dev/null'])
    b_out.wait() # in case the driver warms up very slowly
    time.sleep(2)
    subprocess.Popen(['sudo', modifier_path, hex(pte_pa), hex(pte_val)])
