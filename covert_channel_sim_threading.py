...
def load_cuda_libs():
    ...
    if libcuda is None:
        try:
            ...
            libcuda.cuCtxPushCurrent_v2.argtypes = [ctypes.c_void_p]; libcuda.cuCtxPushCurrent_v2.restype = int
            libcuda.cuCtxPopCurrent_v2.argtypes = [ctypes.POINTER(ctypes.c_void_p)]; libcuda.cuCtxPopCurrent_v2.restype = int
...
def sender_thread(bit_q, delay_ms, stop_event):
    global pool_gpu_va_start, pages0_indices, pages1_indices, cuda_context, sender_module
    ...
    try:
        ...
        CU_CHECK(libcuda.cuCtxPushCurrent_v2(cuda_context), "Sender cuCtxPushCurrent")
        
        # Get kernel function
        CU_CHECK(libcuda.cuModuleGetFunction(ctypes.byref(sender_kernel_func), sender_module, b"sender_contention_kernel"), "Sender cuModuleGetFunction")
        ...
        d_page_vas_val = ctypes.c_ulonglong(d_page_vas_ptr.value)
        n_pages_val = ctypes.c_int(N_PAGES)
        stop_flag_kernel_arg = ctypes.c_ulonglong(stop_flag_gpu_ptr.value)
        ...
        while not stop_event.is_set():
            try:
                ...
                CU_CHECK(libcuda.cuMemcpyHtoD_v2(d_page_vas_ptr.value,
                                                 ctypes.byref(h_page_vas),
                                                 ctypes.sizeof(h_page_vas)),
                         f"Sender memcpy page_vas bit {bit}")

                # Prepare launch params
                launch_args = [
                    ctypes.byref(d_page_vas_val),
        ...
        if d_page_vas_ptr.value != 0:
            try: CU_CHECK(libcuda.cuMemFree_v2(d_page_vas_ptr.value), "Sender cuMemFree page_vas")
            except Exception: pass
        if stop_flag_gpu_ptr.value != 0:
            try: CU_CHECK(libcuda.cuMemFree_v2(stop_flag_gpu_ptr.value), "Sender cuMemFree stop_flag")
            except Exception: pass
        # Pop context
        try:
            ctx_ptr = ctypes.c_void_p()
            CU_CHECK(libcuda.cuCtxPopCurrent_v2(ctypes.byref(ctx_ptr)), "Sender cuCtxPopCurrent")
        except Exception: pass
        print("[Sender] Thread finished.")


def receiver_thread(result_q, num_bits, samples_per_bit, stop_event):
    global pool_gpu_va_start, pages0_indices, pages1_indices, cuda_context, receiver_module
    ...
    try:
        # Push context for this thread
        CU_CHECK(libcuda.cuCtxPushCurrent_v2(cuda_context), "Receiver cuCtxPushCurrent")
        
        # Get kernel function
        CU_CHECK(libcuda.cuModuleGetFunction(ctypes.byref(receiver_kernel_func), receiver_module, b"receiver_probe_kernel"), "Receiver cuModuleGetFunction")
        ...
        if d_page_vas0_ptr.value != 0:
            try: CU_CHECK(libcuda.cuMemFree_v2(d_page_vas0_ptr.value), "Receiver cuMemFree page_vas0")
            except Exception: pass
        if d_page_vas1_ptr.value != 0:
            try: CU_CHECK(libcuda.cuMemFree_v2(d_page_vas1_ptr.value), "Receiver cuMemFree page_vas1")
            except Exception: pass
        if results_buffer_gpu_va != 0:
            try: CU_CHECK(libcuda.cuMemFree_v2(results_buffer_gpu_va), "Receiver cuMemFree results")
            except Exception: pass
        # Pop context
        try:
            ctx_ptr = ctypes.c_void_p()
            CU_CHECK(libcuda.cuCtxPopCurrent_v2(ctypes.byref(ctx_ptr)), "Receiver cuCtxPopCurrent")
        except Exception: pass
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Load PTX & Kernels
        sender_module_ptr = ctypes.c_void_p()
        receiver_module_ptr = ctypes.c_void_p()
        ...
        CU_CHECK(libcuda.cuModuleLoadData(ctypes.byref(sender_module_ptr), ptx_content_null_terminated), "cuModuleLoadData sender")
        CU_CHECK(libcuda.cuModuleLoadData(ctypes.byref(receiver_module_ptr), ptx_content_null_terminated), "cuModuleLoadData receiver")
        sender_module = sender_module_ptr
        receiver_module = receiver_module_ptr

        initialization_ok = True

    except Exception as e: print(f"Error during setup: {e}"); initialization_ok = False

    if not initialization_ok:
        print("Initialization failed. Cleaning up...")
        if cuda_context and cuda_context.value:
            try: CU_CHECK(libcuda.cuCtxDestroy_v2(cuda_context), "CtxDestroy on Error")
            except Exception: pass
        if pool_gpu_va_start != 0:
            try: CU_CHECK(libcuda.cuMemFree_v2(pool_gpu_va_start), "MemFree on Error")
            except Exception: pass
        exit(1)
    ...
    sender_th = threading.Thread(target=sender_thread, args=(bit_queue, DELAY_MS_SENDER, stop_event))
    receiver_th = threading.Thread(target=receiver_thread, args=(result_queue, NUM_BITS, RECEIVER_SAMPLES_PER_BIT, stop_event))
    ...
    for i, bit in enumerate(random_bits): 
        bit_queue.put(bit)
        time.sleep(DELAY_MS_SENDER / 1000.0 * 1.1)
    
    print("All bits sent. Waiting for receiver...")
    time.sleep(3)
    stop_event.set()
    
    sender_th.join(timeout=10)
    receiver_th.join(timeout=15)
    
    if sender_th.is_alive(): print("Warning: Sender timeout.")
    if receiver_th.is_alive(): print("Warning: Receiver timeout.")
    ...
    try:
        ...
        decoded_bits = []
        samples_per_window = max(1, num_samples_received // NUM_BITS)
        print(f"Decoding with window size: {samples_per_window}")
        for i in range(NUM_BITS):
             start_idx = i * samples_per_window
             end_idx = min((i + 1) * samples_per_window, num_samples_received)
             if start_idx >= end_idx: decoded_bits.append(-1); continue
             window = results_pairs[start_idx:end_idx]
             t0 = window[:, 0]; t1 = window[:, 1]
             m0 = np.sum(t0 > THRESHOLD_T); m1 = np.sum(t1 > THRESHOLD_T)
             h0 = len(window) - m0; h1 = len(window) - m1
        ...
        print("\n--- Results ---")
        print(f"Sent {len(random_bits)} bits.")
        print(f"Decoded {valid} bits.")
        print(f"Correct: {correct}/{valid}")
        print(f"Accuracy: {accuracy:.2f}%")
        print(f"Receiver Duration: {total_duration:.2f} s")
        print(f"Estimated Bandwidth: {bandwidth:.2f} bps ({bandwidth/1000:.2f} kbps)")

        try:
            import matplotlib.pyplot as plt
            plt.figure(figsize=(15, 5))
            plt.plot(results_pairs[:, 0], label='t0', alpha=0.7)
            plt.plot(results_pairs[:, 1], label='t1', alpha=0.7)
            plt.axhline(THRESHOLD_T, color='r', linestyle='--', label=f'Threshold ({THRESHOLD_T})')
            plt.title('Covert Channel Receiver Timing')
            plt.xlabel('Sample Index')
            plt.ylabel('Cycles')
            plt.legend()
            plt.ylim(bottom=0, top=max(2000, np.max(results_pairs)*1.1 if num_samples_received > 0 else 2000))
            plt.grid(True)
            plt.savefig("covert_channel_timing_threading.png")
            print("Saved timing plot to covert_channel_timing_threading.png")
        except ImportError: print("matplotlib not found, skipping plot.")
        except Exception as plot_e: print(f"Error plotting: {plot_e}")

    except (queue.Empty, ValueError, IndexError) as qe: 
        print(f"Error: No results or processing failed: {qe}")
    except Exception as e: 
        print(f"An unexpected error processing results: {e}")

    print("Cleaning up...")
    if pool_gpu_va_start != 0:
        try:
            CU_CHECK(libcuda.cuMemFree_v2(pool_gpu_va_start), "cuMemFree Pool")
            print("Memory pool freed.")
        except Exception as e: print(f"Error during pool cleanup: {e}")
    if cuda_context and cuda_context.value:
        try:
            CU_CHECK(libcuda.cuCtxDestroy_v2(cuda_context), "cuCtxDestroy")
            print("Context destroyed.")
        except Exception as e: print(f"Error cleaning context: {e}")
    print("Done.")
