...
__global__ void probe_time_kernel2(unsigned long long *target_vas, int num_pages,
                                   ...
                                   unsigned long long *result_time,
                                   unsigned long long *thread_timing_buffer) {
...
__global__ void matrix_mul_kernel(float *A, float *B, float *C, int N,
                                  unsigned long long *thread_timing_buffer, int timing_offset) {
...
__global__ void matrix_add_kernel(float *C, float *D, float *E, int elements,
                                  unsigned long long *thread_timing_buffer, int timing_offset) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned long long start_time = clock64();
    if (idx < elements) {
