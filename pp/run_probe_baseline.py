...
# import os
# import sysnvtop
# # Import specific functions and constants from gpu_utils_pp
...
# # --- Configuration ---
# GPU_ID = 0
# TARGET_SET_ID = 5 # Which L3 set index to prime/probe
# NUM_SAMPLES = 2000 # Number of hit/miss measurements to take
# ALLOC_POOL_MB = 8 * 1024 # 8 GB pool for finding pages
...
#     # Use managed alloc for result
#     result_time_gpu_ptr = ctypes.c_ulonglong(0) # Pointer object from managed alloc
#     result_time_gpu_va = 0 # The actual VA/Device Ptr value
...
#         # Use the managed VA for result time kernel argument
...
#         # --- Measurement Loop ---
#         # No separate host variable, read directly from managed memory after sync
...
#         print(f"Starting {NUM_SAMPLES} Hit/Miss measurements...")
...
#             # Read directly from managed memory VA
...
#             # Perform L3 Flush
#             CU_CHECK(local_libcuda.cuLaunchKernel(flush_kernel_func, 1, 1, 1, 1, 1, 1, 0, None, flush_params, None), f"Launch Flush {i}")
#             CU_CHECK(local_libcuda.cuStreamSynchronize(None), f"Sync Flush {i}")
...
#             # Read directly from managed memory VA
#             # Re-cast just in case, though pointer should be stable
#             result_time_ptr_host_view = ctypes.cast(result_time_gpu_va, ctypes.POINTER(ctypes.c_ulonglong))
...
from gpu_utils_pp import (load_cuda_libs_pp, CU_CHECK, CUDA_RT_CHECK,
                          compile_kernels_to_ptx_pp, find_pages_for_set,
                          PAGE_SIZE, PRIME_PROBE_N, L3_FILL_COUNT,
                          L1_L2_EVICT_COUNT) # Added L1_L2_EVICT_COUNT

# --- Configuration ---
GPU_ID = 0
TARGET_SET_ID = 0 # Which L3 set index to prime/probe
NUM_SAMPLES = 2000
ALLOC_POOL_MB = 8 * 1024 # 8 GB pool
KERNELS_FILE = "pp_baseline_kernels.cu"
KERNELS_PTX = "pp_baseline_kernels.ptx"
...
def main():
    ...
    driver_ok, local_libcuda, local_libcudart = load_cuda_libs_pp()
    if not driver_ok: print("Failed loading libcuda."); exit(1)
    if not local_libcudart: print("Warning: libcudart not loaded.") # Needed for sync

    # --- Compile Kernels ---
    if not compile_kernels_to_ptx_pp(KERNELS_FILE, KERNELS_PTX): exit(1)

    cu_context = None; cu_module = None; pool_gpu_va_start = 0
    d_prime_probe_vas_ptr = ctypes.c_ulonglong(0)
    d_flush_vas_ptr = ctypes.c_ulonglong(0)
    d_l1l2_evict_vas_ptr = ctypes.c_ulonglong(0) # <<< ADDED: Pointer for L1/L2 VAs
    ...
    try:
        # --- Init CUDA & Context ---
        CU_CHECK(local_libcuda.cuInit(0), "Main cuInit"); # ... create context ...
        cu_device = ctypes.c_int(); CU_CHECK(local_libcuda.cuDeviceGet(ctypes.byref(cu_device), GPU_ID), "Main cuDeviceGet")
        cu_context_ptr = ctypes.c_void_p(); CU_CHECK(local_libcuda.cuCtxCreate_v2(ctypes.byref(cu_context_ptr), 0, cu_device), "Main cuCtxCreate")
        cu_context = cu_context_ptr
        print("CUDA Context created.")

        # --- Load PTX Module & Kernels ---
        cu_module_ptr = ctypes.c_void_p(); # ... load ptx ...
        with open(KERNELS_PTX, 'rb') as f: ptx_content = f.read()
        ptx_content_null_terminated = ctypes.create_string_buffer(ptx_content + b'\x00')
        CU_CHECK(local_libcuda.cuModuleLoadData(ctypes.byref(cu_module_ptr), ptx_content_null_terminated), "Main cuModuleLoadData")
        cu_module = cu_module_ptr
        probe_kernel_func = ctypes.c_void_p(); flush_kernel_func = ctypes.c_void_p()
        CU_CHECK(local_libcuda.cuModuleGetFunction(ctypes.byref(probe_kernel_func), cu_module, b"probe_time_kernel"), "Main GetFunc probe")
        CU_CHECK(local_libcuda.cuModuleGetFunction(ctypes.byref(flush_kernel_func), cu_module, b"flush_kernel_pp"), "Main GetFunc flush")
        print("Kernels loaded.")
        ...
        # Find pages for Prime/Probe set
        prime_probe_vas = find_pages_for_set(local_libcuda, pool_gpu_va_start, pool_size_bytes, TARGET_SET_ID, PRIME_PROBE_N)
        if len(prime_probe_vas) < PRIME_PROBE_N: raise RuntimeError(f"Could not find enough pages for P+P set {TARGET_SET_ID}")

        # Find pages for L3 flush
        flush_set_id = (TARGET_SET_ID + 5) % 256
        flush_vas_list = find_pages_for_set(local_libcuda, pool_gpu_va_start, pool_size_bytes, flush_set_id, L3_FILL_COUNT)
        # <<< ADDED: Find pages for L1/L2 Eviction set >>>
        l1l2_evict_set_id = (TARGET_SET_ID + 10) % 256 # Use yet another set
        ...
        # Supplement flush list if needed
        if len(flush_vas_list) < L3_FILL_COUNT:
             print(f"Warning: Supplementing L3 flush list.")
             available_pages = pool_size_bytes // PAGE_SIZE; needed = L3_FILL_COUNT - len(flush_vas_list)
             current_existing_vas = set(prime_probe_vas + flush_vas_list + l1l2_evict_vas)
             start_idx = 0
             added_count = 0
             while added_count < needed and start_idx < available_pages:
                  candidate_va = pool_gpu_va_start + start_idx * PAGE_SIZE
                  if candidate_va not in current_existing_vas:
                       flush_vas_list.append(candidate_va)
                       current_existing_vas.add(candidate_va)
                       added_count += 1
                  start_idx += 1
             flush_vas_list = flush_vas_list[:L3_FILL_COUNT]
        print(f"Using {len(flush_vas_list)} pages for L3 flush.")
        if len(flush_vas_list) == 0: raise RuntimeError("Cannot proceed with 0 flush pages.")
        ...
        # <<< ADDED: Allocate and Copy L1/L2 Eviction VAs >>>
        VasListTypeL1L2 = ctypes.c_ulonglong * L1_L2_EVICT_COUNT
        h_l1l2_vas = VasListTypeL1L2(*l1l2_evict_vas)
        CU_CHECK(local_libcuda.cuMemAlloc_v2(ctypes.byref(d_l1l2_evict_vas_ptr), ctypes.sizeof(h_l1l2_vas)), "Main Alloc L1L2 VAs")
        CU_CHECK(local_libcuda.cuMemcpyHtoD_v2(d_l1l2_evict_vas_ptr.value, ctypes.byref(h_l1l2_vas), ctypes.sizeof(h_l1l2_vas)), "Main Memcpy L1L2 VAs")
        # <<< -------------------------------------------- >>>


        # Allocate result time buffer using Managed Alloc
        CU_CHECK(local_libcuda.cuMemAllocManaged(ctypes.byref(result_time_gpu_ptr), ctypes.sizeof(ctypes.c_ulonglong), flags), "Main cuMemAllocManaged Result Time")
        result_time_gpu_va = result_time_gpu_ptr.value
        if not result_time_gpu_va: raise RuntimeError("Result time buffer alloc failed")
        print("Device memory setup complete.")
        ...
        num_pp_pages_val = ctypes.c_int(PRIME_PROBE_N)
        num_flush_pages_val = ctypes.c_int(len(flush_vas_list))
        num_l1l2_evict_val = ctypes.c_int(L1_L2_EVICT_COUNT) # <<< ADDED
        d_pp_vas_val = ctypes.c_ulonglong(d_prime_probe_vas_ptr.value)
        d_flush_vas_val = ctypes.c_ulonglong(d_flush_vas_ptr.value)
        ...
        probe_args = [
            ...
            ctypes.byref(num_l1l2_evict_val),   # <<< ADDED
            ctypes.byref(d_result_time_kernel_arg)
        ]
        probe_params = (ctypes.c_void_p * len(probe_args))(*[ctypes.cast(p, ctypes.c_void_p) for p in probe_args])

        # Args for flush_kernel_pp: l3_fill_pages_va, num_l3_fill
        flush_args = [ ctypes.byref(d_flush_vas_val), ctypes.byref(num_flush_pages_val) ]
        flush_params = (ctypes.c_void_p * len(flush_args))(*[ctypes.cast(p, ctypes.c_void_p) for p in flush_args])


        # --- Measurement Loop ---
        print("Performing initial prime (includes L1/L2 flush)...")
        CU_CHECK(local_libcuda.cuLaunchKernel(probe_kernel_func, 1, 1, 1, 1, 1, 1, 0, None, probe_params, None), "Launch Prime")
        CU_CHECK(local_libcuda.cuStreamSynchronize(None), "Sync Prime")

        print(f"Starting {NUM_SAMPLES} Hit/Miss measurements...")
        for i in range(NUM_SAMPLES):
            # Measure Hit (Kernel now includes L1/L2 flush before probe)
            CU_CHECK(local_libcuda.cuLaunchKernel(probe_kernel_func, 1, 1, 1, 1, 1, 1, 0, None, probe_params, None), f"Launch Probe Hit {i}")
            CU_CHECK(local_libcuda.cuStreamSynchronize(None), f"Sync Probe Hit {i}")
            result_time_ptr_host_view = ctypes.cast(result_time_gpu_va, ctypes.POINTER(ctypes.c_ulonglong))
            hit_timings.append(result_time_ptr_host_view.contents.value)
            ...
            # Measure Miss (Kernel now includes L1/L2 flush before probe)
            CU_CHECK(local_libcuda.cuLaunchKernel(probe_kernel_func, 1, 1, 1, 1, 1, 1, 0, None, probe_params, None), f"Launch Probe Miss {i}")
            CU_CHECK(local_libcuda.cuStreamSynchronize(None), f"Sync Probe Miss {i}")
            result_time_ptr_host_view = ctypes.cast(result_time_gpu_va, ctypes.POINTER(ctypes.c_ulonglong))
            miss_timings.append(result_time_ptr_host_view.contents.value)
        ...
        print("\nCleaning up...")
        # <<< ADDED: Free L1/L2 eviction VA list >>>
        if d_prime_probe_vas_ptr.value != 0: 
            try: CU_CHECK(local_libcuda.cuMemFree_v2(d_prime_probe_vas_ptr.value), "...") 
            except Exception: pass
        if d_flush_vas_ptr.value != 0: 
            try: CU_CHECK(local_libcuda.cuMemFree_v2(d_flush_vas_ptr.value), "...") 
            except Exception: pass
        if d_l1l2_evict_vas_ptr.value != 0: 
            try: CU_CHECK(local_libcuda.cuMemFree_v2(d_l1l2_evict_vas_ptr.value), "Free L1L2 VAs") 
            except Exception: pass
        if result_time_gpu_va != 0: 
            try: CU_CHECK(local_libcuda.cuMemFree_v2(result_time_gpu_va), "...") 
            except Exception: pass
        if pool_gpu_va_start != 0: 
            try: CU_CHECK(local_libcuda.cuMemFree_v2(pool_gpu_va_start), "...") 
            except Exception: pass
        if cu_context and cu_context.value: 
            try: CU_CHECK(local_libcuda.cuCtxDestroy_v2(cu_context), "..."); print("Context destroyed.") 
            except Exception: pass
        # <<< ------------------------------------- >>>
        print("Cleanup finished.")
